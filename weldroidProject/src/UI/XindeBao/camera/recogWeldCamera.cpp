...
RecogWeldCamera::RecogWeldCamera():
...
void RecogWeldCamera::connectDevice()
{
...
void RecogWeldCamera::disconnectDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS RecogWeldCamera::OpenDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS RecogWeldCamera::CloseDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS RecogWeldCamera::InitDevice()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS RecogWeldCamera::SetPixelFormat8bit()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
GX_STATUS RecogWeldCamera::GetDeviceInitParam()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
bool RecogWeldCamera::PrepareForShowImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    bool ret = false;
...
bool  RecogWeldCamera::PrepareForShowColorImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    //创建QImage对象，用于显示图像
...
*/
//----------------------------------------------------------------------------------
bool  RecogWeldCamera::PrepareForShowMonoImg()
{
    //创建QImage对象，用于显示图像
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    m_show_image = new QImage(m_image_width, m_image_height, QImage::Format_RGB888);
...
void RecogWeldCamera::UnPrepareForShowImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    if(m_frame_data.pImgBuf != NULL)
    {
...
cv::Mat RecogWeldCamera::getImg()
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
    return const_cast<RecogWeldCamera*>(this)->takePicture() ;
}

GX_FRAME_DATA RecogWeldCamera::getFrameDate()
{
    return m_frame_data;
}

cv::Mat RecogWeldCamera::takePicture()
{
...
void RecogWeldCamera::ShowErrorString(GX_STATUS error_status)
{
