...
void AdjustCamera::connectDevice()
{
...
GX_STATUS AdjustCamera::OpenDevice()
{
    ...
    open_param.pszContent  = "RH1899008016";
...
void AdjustCamera::ShowErrorString(GX_STATUS error_status)
{
...
void AdjustCamera::setExposureTime(int exposureValue)
{
    std::lock_guard<std::recursive_mutex> lck(_mtx);
...
void AdjustCamera::setGain( float f )
{
    std::lock_guard<std::recursive_mutex> lck(_mtx) ;
    if(m_device_handle == nullptr)
        return;
...
void AdjustCamera::setRoi( float a, float b,int c,int d)
{
    std::lock_guard<std::recursive_mutex> lck(_mtx) ;
    if(m_device_handle == nullptr)
        return;
